...
DATASET_DIR = "XXX/EhrAgent/data/ehrsql-ehragent/mimic_iii/"

def db_loader(target_ehr):
    ehr_dict = {"admissions":DATASET_DIR+"ADMISSIONS.csv",
                "chartevents":DATASET_DIR+"CHARTEVENTS.csv",
                "cost":DATASET_DIR+"COST.csv",
                "d_icd_diagnoses":DATASET_DIR+"D_ICD_DIAGNOSES.csv",
                "d_icd_procedures":DATASET_DIR+"D_ICD_PROCEDURES.csv",
                "d_items":DATASET_DIR+"D_ITEMS.csv",
                "d_labitems":DATASET_DIR+"D_LABITEMS.csv",
                "diagnoses_icd":DATASET_DIR+"DIAGNOSES_ICD.csv",
                "icustays":DATASET_DIR+"ICUSTAYS.csv",
                "inputevents_cv":DATASET_DIR+"INPUTEVENTS_CV.csv",
                "labevents":DATASET_DIR+"LABEVENTS.csv",
                "microbiologyevents":DATASET_DIR+"MICROBIOLOGYEVENTS.csv",
                "outputevents":DATASET_DIR+"OUTPUTEVENTS.csv",
                "patients":DATASET_DIR+"PATIENTS.csv",
                "prescriptions":DATASET_DIR+"PRESCRIPTIONS.csv",
                "procedures_icd":DATASET_DIR+"PROCEDURES_ICD.csv",
                "transfers":DATASET_DIR+"TRANSFERS.csv",
                }
...
def sql_interpreter(command):
    con = sqlite3.connect(DATASET_DIR+"mimic_iii.db")
    cur = con.cursor()
...
if __name__ == "__main__":
    ...
    dataset = db_loader("microbiologyevents")
